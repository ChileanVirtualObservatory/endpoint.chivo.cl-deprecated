...
class ElasticQuery():
	...
	def send_query(self, query, timeout=180):
		...
		if self.query == -1:
			...
			#a.pprint(self.query)
			data = self.__connection.search(index=self.__primary_index, doc_type=self.__primary_mapping, body=json.dumps(self.query), size=self.__max_result_size, request_timeout=timeout)
			# a.pprint(data)

			query_size = data["hits"]["total"]
			query_time = data["took"]
			query_data = data["hits"]["hits"]
			query_real_size = len(query_data)
			...
			return {"results": filtered_data, "time": query_time, "total": query_real_size, "metadata": metadata}
